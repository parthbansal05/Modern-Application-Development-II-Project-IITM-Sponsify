...
export default {
	...
	async created() {
		try {
			const response = await axios.get('http://localhost:5000/sponsor/search_influencer', {
	...
	methods: {
		send_request(influencer_id) {
			try {
				// search budger for the influencer with influencer_id
				console.log(this.selectedCampaign);
				const budgetIndex = this.campaigns[0].indexOf(this.selectedCampaign);
				const budgetValue = budgetIndex !== -1 ? this.campaigns[2][budgetIndex] : null;

				axios.get('http://localhost:5000/sponsor/initiate_chat/' + this.selectedCampaign + '/' + influencer_id + '/' + budgetValue, {
					headers: {
